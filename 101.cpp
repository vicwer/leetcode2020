...
bool isSymmetric(BinaryTree* root1, BinaryTree* root2)
{
    if(!root1 && !root2)
    {
        return true;
    ...
    if(root1 && root2)
    {
        std::cout << root1->val << " " << root2->val << std::endl;
...
int main()
{
    std::vector<std::vector<int> > list = {{1,2,3}, {1,2,3}, {1}, {2}, {1,2}, {1,-233,2}};
    CreateTree tree;
    for(int i = 0; i < list.size(); i+=2)
    {
        BinaryTree* root1 = tree.createTree(list[i]);
        BinaryTree* root2 = tree.createTree(list[i+1]);
        std::cout << isSymmetric(root1, root2) << std::endl;
    }
